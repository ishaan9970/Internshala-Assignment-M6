...
# Form implementation generated from reading ui file 'Assignment-M6.ui'
#
...
"background-color:#D3D3D3")
    ...
    def findprice(self):
        ...
        bookcursor=bookstore.cursor()
        book_name=self.t1.text()
        sql="SELECT price FROM books WHERE name = '"+book_name+"';"
        bookcursor.execute(sql)
        price=bookcursor.fetchone()
        if price!=None:
            self.t2.setText("Rs."+str(price[0]))
        else:
    ...
    def totalPrice(self):
        ...
        bookcursor.execute(sql)
        price=bookcursor.fetchone()
        qty=int(self.t3.text())
        total_price=qty*price[0]
        self.t4.setText(str(total_price))
